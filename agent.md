...
当前阶段重点不是完整导航，而是先把“路线记录能力”做稳定，并提供可独立验证腰带硬件的 Debug 工具：
...
- Debug 构建下的 BLE 腰带连接和单电机手动测试

当前已完成路线切线的纯算法计算、保存详情页可视化，以及调试用途的 BLE 单电机控制。正式运动流程尚未接入路线匹配、腰带方向识别和自动震动反馈。
...
当前运动页仍是占位入口。Debug 构建中在 2.5 秒内连续点击底部“运动”入口 5 次，可打开测试工具页；该页面包含虚拟 GPS 和 BLE 腰带单电机轮盘。
...
4. 不要在 model 层写业务逻辑。
5. Debug 电机控制只用于硬件验证，不要直接扩展成未经验证的自动导航反馈。
6. 新增功能必须优先考虑能不能被测试。
